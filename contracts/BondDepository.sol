...
contract BondDepository is Ownable, IBond {
  ...
  address public immutable SATO; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints SATO when receives principle
  ...
  struct Bond {
    uint256 payout; // SATO remaining to be paid
  ...
  function deposit(
    ...
    if (fee != 0) {
      ...
      IERC20(SATO).safeTransfer(DAO, fee);
    }
   ...
   *  @return uint
   */
  function redeem(address _recipient, bool _stake) external override returns (uint256) {
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(SATO).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(SATO).approve(stakingHelper, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(SATO).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to SATO supply
  ...
  /**
   *  @notice calculate amount of SATO available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or SATO) to the DAO
   *  @return bool
   */
